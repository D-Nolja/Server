...
public class ShortestController {
    ...
    @GetMapping()
    public ShortestListDto recommendShortestLocation(@ModelAttribute ShortestRequestDto request){
        ...
        log.info("[ShortestController recommendShortestLocation] request :: request={}", request);
        List<ShortestItemDto> result = recommendShortestService.recommendLocationFrom(x,y,limit, maxCount);
        if(result.size() ==0){
